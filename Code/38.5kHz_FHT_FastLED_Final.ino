...
#define OCTAVE 1 //configures output method to be fht_oct_out(), explained further in report
...
bool debug_samples = false, debug_new_bands = false;
...
void loop() { 
  if (sampleCount >= FHT_N - 1) { //Wait for 256 samples to be collected before starting the processing of the samples
    if (showSampleRate) {
      Serial.println((float)256/(sampleEndTime - sampleStartTime)*1000);
    }
    if (freqBinsRepresentation) {
      ...
      byte LEDsToShowAvg = sampleAverage*(N_LEDs/2)/(255/3);
...
void LedColors() { 
  ...
  if (freqBinsRepresentation) {
    ...
    for (int i = 0; i < N_LEDs/n_sections; i++) {
      if (i < 6) {
        colors[i] = colors[23-i] = CHSV(85, 255, 0);
      } else if (i < 10){
        colors[i] = colors[23-i] = CHSV(42, 255, 0);
      } else {
        colors[i] = colors[23-i] = CHSV(0, 255, 0);
      }
    }
  }
}
